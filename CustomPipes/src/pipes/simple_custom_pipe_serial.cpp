...
void Payload_PipeTest()
{
	...
	// ---
	PayloadOrError	pe { Payload { "just to test it", 13 } };		// to test with and without "*" in operator |
	auto cos = * pe;
	PayloadOrError second_pe = cos;
	// ---
...
void Payload_PipeTest_Monadic()
{
	...
	auto test = Payload_Proc_1( Payload { "Start string ", 42 } );
